...
// the top-left corner of an A4 page (210×297mm). A field may appear multiple
// times in a template (e.g. the signing block repeats on several pages); each
// occurrence is stored as its own row, distinguished by OccurrenceIndex.
type TemplateFieldPosition struct {
	...
	OccurrenceIndex int            `gorm:"not null;default:1" json:"occurrence_index"`
