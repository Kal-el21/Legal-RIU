...
import { Scale, LayoutDashboard, FileText, Menu, ChevronRight, ChevronLeft } from 'lucide-react'
import { useState } from 'react'
import { cn } from '@/lib/utils'
import SidebarUserButton from '@/components/common/SidebarUserButton'

const NAV = [
  { label: 'Dashboard', href: '/legal-au', icon: LayoutDashboard, exact: true },
  { label: 'Manajemen Kasus', href: '/legal-au/cases', icon: Scale },
  { label: 'Materi Legal', href: '/legal-au/materials', icon: FileText },
]

export default function LegalAULayout() {
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#f8fafc' }}>
      <aside className={cn(
          ...
          <Link to="/" className="flex items-center gap-2.5">
            ...
            {!sidebarCollapsed && (
              <div>
                ...
                <p className="text-xs text-white/40 leading-none">Legal AU Panel</p>
              </div>
            )}
          </Link>
        </div>
        ...
        <SidebarUserButton settingsPath="/legal-au/settings" dark={true} collapsed={sidebarCollapsed} />
      </aside>
