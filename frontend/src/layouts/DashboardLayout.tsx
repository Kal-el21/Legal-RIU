...
import { Scale, LayoutDashboard, FileText, FileSearch, LogOut, Menu, X, ChevronRight } from 'lucide-react'
      ...
      <aside className={cn(
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          {NAV.map((item) => (
            <Link key={item.href} to={item.href}
              ...
              style={isActive(item.href) ? { background: '#C8102E' } : {}}>
              <item.icon className="w-4 h-4 flex-shrink-0" />
              {item.label}
              {isActive(item.href) && <ChevronRight className="w-3.5 h-3.5 ml-auto opacity-70" />}
            </Link>
          ))}
        </nav>
        ...
        <div className="p-3 border-t border-gray-100">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gray-50 mb-1">
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style={{ background: '#0B2545' }}>
              {user?.full_name?.charAt(0).toUpperCase()}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-gray-900 truncate">{user?.full_name}</p>
              <p className="text-xs text-gray-500 truncate">{user?.division}</p>
            </div>
          </div>
