...
import { Scale, LayoutDashboard, FileText, FileSearch, Users, Menu, ChevronRight } from 'lucide-react'
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          {NAV.map((item) => (
            <Link key={item.href} to={item.href} onClick={() => setSidebarOpen(false)}
              ...
              {item.label}
              {isActive(item.href, item.exact) && <ChevronRight className="w-3.5 h-3.5 ml-auto opacity-60" />}
            </Link>
          ))}
        </nav>

        <SidebarUserButton settingsPath="/admin/settings" dark={true} />
      </aside>
