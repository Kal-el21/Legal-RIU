...
import { useLegalOpinion, useLegalUpdateStatus, useAdminDownloadPDF } from '@/hooks/useLegalOpinion'
import { useAuthStore } from '@/store/auth.store'
import { useQueryClient, useMutation } from '@tanstack/react-query'
import SharedLegalOpinionDetailPage from '@/components/shared/LegalOpinionDetailPage'
import { validateFile } from '@/lib/utils'
import { legalOpinionService } from '@/services/legal-opinion.service'

export default function LegalLegalOpinionDetailPage() {
  const { id } = useParams<{ id: string }>()
  const qc = useQueryClient()
  const hasPermission = useAuthStore((state) => state.hasPermission)

  const { data: lo } = useLegalOpinion(id!)
  const updateStatus = useLegalUpdateStatus()
  ...
  const nextOptions = useMemo(() => {
    ...
    return NEXT_STATUSES[lo?.status ?? ''] ?? []
  }, [lo?.status])
  ...
  const uploadResultMutation = useMutation({
    mutationFn: ({ file, notes }: { file: File; notes: string }) =>
      legalOpinionService.legalUploadResult(id!, file, notes),
    onSuccess: () => {
  ...
  const actionCards = (
    <>
      ...
      {canUploadResult && (
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          <h3 className="text-sm font-semibold mb-4" style={{ color: '#0B2545' }}>Upload Hasil Kajian</h3>
  ...
  return (
    <SharedLegalOpinionDetailPage
      showDownloadPDF={canDownloadPDF}
      downloadPDFMutation={downloadPDF as any}
    >
      {actionCards}
    </SharedLegalOpinionDetailPage>
  )
