...
export default function ExternalDashboardPage() {
  const { data: stats } = useQuery({
    queryKey: ['dashboard', 'external', 'stats'],
    queryFn: dashboardService.getExternalStats,
  })

  const { data: recent } = useQuery({
    queryKey: ['dashboard', 'external', 'recent'],
    queryFn: dashboardService.getExternalRecent,
  })
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold" style={{ color: '#0B2545' }}>External Dashboard</h1>
        <p className="text-sm text-gray-500 mt-0.5">Lihat dan unduh hasil Legal Opinion serta Review Dokumen</p>
      </div>
      ...
      {/* Recent submissions - view only, no edit */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-50">
            ...
            <Link to="/external/legal-opinions" className="text-xs font-medium flex items-center gap-1 hover:underline" style={{ color: '#C8102E' }}>
          ...
          <div className="divide-y divide-gray-50">
            {!recent?.legal_opinions?.length ? (
              ...
              <Link key={lo.id} to={`/external/legal-opinions/${lo.id}`}
        ...
        <div className="bg-white rounded-2xl border border-gray-100">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-50">
            ...
            <Link to="/external/review-documents" className="text-xs font-medium flex items-center gap-1 hover:underline" style={{ color: '#C8102E' }}>
              Lihat <ArrowRight className="w-3 h-3" />
            </Link>
          </div>
          <div className="divide-y divide-gray-50">
            {!recent?.document_reviews?.length ? (
              ...
              <Link key={dr.id} to={`/external/review-documents/${dr.id}`}
                className="flex items-center gap-3 px-6 py-3.5 hover:bg-gray-50/50 transition-colors">
                <div className="flex-1 min-w-0">
