...
import { useCedants, useCreateCedant, useImportCedants } from '@/hooks/useLegalCase'
import { legalCaseService } from '@/services/legal-case.service'
import ImportCard from '@/components/common/ImportCard'
import api from '@/services/api'
import type { Cedant } from '@/types'
...
export default function CedantManagementPage() {
  const [search, setSearch] = useState('')
  const [modal, setModal] = useState<'create' | 'edit' | null>(null)
  const [selected, setSelected] = useState<Cedant | null>(null)

  const { data, isLoading } = useCedants({ limit: 200 })
  const createMutation = useCreateCedant()
  const importMutation = useImportCedants()

  const form = useForm<FormData>({ resolver: zodResolver(schema), defaultValues: { name: '', description: '' } })

  const openEdit = (item: Cedant) => {
    setSelected(item)
  ...
  const onEditSubmit = async (data: FormData) => {
    if (!selected) return
    await api.put(`/admin/cedants/${selected.id}`, data)
    closeModal()
    window.location.reload()
  }

  const handleDelete = async (item: Cedant) => {
    if (!confirm(`Hapus ${item.name}?`)) return
    await api.delete(`/admin/cedants/${item.id}`)
    window.location.reload()
  }

  const filtered = data?.filter((c) => c.name.toLowerCase().includes(search.toLowerCase())) ?? data ?? []

  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: '#0B2545' }}>Cedant</h1>
          <p className="text-sm text-gray-500 mt-0.5">Kelola data cedant</p>
        </div>
      ...
      <ImportCard
        ...
        onDownloadTemplate={() => legalCaseService.downloadCedantTemplate()}
      />
      ...
      {modal === 'create' && (
        <Modal title="Tambah Cedant" onClose={closeModal}>
          <form onSubmit={form.handleSubmit(onCreateSubmit)} className="space-y-4">
            <Field label="Nama" error={form.formState.errors.name?.message}>
              <Input {...form.register('name')} placeholder="Nama cedant" />
            </Field>
