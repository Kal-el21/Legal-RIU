...
export default function PurposeTypeManagementPage() {
  const [search, setSearch] = useState('')
  const [modal, setModal] = useState<'create' | 'edit' | null>(null)
  const [selected, setSelected] = useState<PurposeType | null>(null)

  const { data, isLoading } = usePurposeTypes()
  ...
  const openEdit = (item: PurposeType) => {
    setSelected(item)
    form.reset({ name: item.name, description: item.description || '', is_active: item.is_active })
    setModal('edit')
  }

  const closeModal = () => { setModal(null); setSelected(null); form.reset({ name: '', description: '', is_active: true }) }
  ...
  const handleDelete = async (item: PurposeType) => {
    if (!confirm(`Hapus ${item.name}?`)) return
    await deleteMutation.mutateAsync(item.id)
  }

  const filtered = data?.filter((pt) => pt.name.toLowerCase().includes(search.toLowerCase())) ?? data ?? []

  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: '#0B2545' }}>Tujuan Pembuatan</h1>
          <p className="text-sm text-gray-500 mt-0.5">Kelola tujuan pembuatan</p>
        </div>
      ...
      {modal === 'create' && (
        <Modal title="Tambah Tujuan Pembuatan" onClose={closeModal}>
          <form onSubmit={form.handleSubmit(onCreateSubmit)} className="space-y-4">
            <Field label="Nama" error={form.formState.errors.name?.message}>
              <Input {...form.register('name')} placeholder="Nama tujuan pembuatan" />
            </Field>
