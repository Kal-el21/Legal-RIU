...
export default function CaseCategoryManagementPage() {
  const [search, setSearch] = useState('')
  const [modal, setModal] = useState<'create' | 'edit' | null>(null)
  const [selected, setSelected] = useState<CaseCategory | null>(null)
  ...
  const openEdit = (item: CaseCategory) => {
    setSelected(item)
  ...
  const handleDelete = async (item: CaseCategory) => {
    if (!confirm(`Hapus ${item.label}?`)) return
    await deleteMutation.mutateAsync(item.id)
  }

  const filtered = data?.filter((cc) => cc.label.toLowerCase().includes(search.toLowerCase()) || cc.code.toLowerCase().includes(search.toLowerCase())) ?? data ?? []

  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: '#0B2545' }}>Kategori</h1>
          <p className="text-sm text-gray-500 mt-0.5">Kelola kategori kasus</p>
        </div>
      ...
      {modal === 'create' && (
        <Modal title="Tambah Kategori" onClose={closeModal}>
          <form onSubmit={form.handleSubmit(onCreateSubmit)} className="space-y-4">
            <Field label="Kode" error={form.formState.errors.code?.message}>
              <Input {...form.register('code')} placeholder="Kode kategori" />
            </Field>
            <Field label="Label" error={form.formState.errors.label?.message}>
              <Input {...form.register('label')} placeholder="Label kategori" />
            </Field>
