...
import { useDocumentTypes, useCreateDocumentType, useUpdateDocumentType, useDeleteDocumentType, useImportDocumentTypes } from '@/hooks/useDocumentType'
...
export default function DocumentTypeManagementPage() {
  ...
  const form = useForm<FormData>({ resolver: zodResolver(schema), defaultValues: { name: '', label: '', is_active: true } })

  const openEdit = (item: DocumentType) => {
    setSelected(item)
    form.reset({ name: item.name, label: item.label, is_active: item.is_active })
    setModal('edit')
  }

  const closeModal = () => { setModal(null); setSelected(null); form.reset({ name: '', label: '', is_active: true }) }
  ...
  const filtered = data?.filter((dt) => dt.label.toLowerCase().includes(search.toLowerCase()) || dt.name.toLowerCase().includes(search.toLowerCase())) ?? data ?? []

  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: '#0B2545' }}>Jenis Dokumen</h1>
          <p className="text-sm text-gray-500 mt-0.5">Kelola jenis dokumen untuk review dokumen</p>
        </div>
      ...
      <ImportCard
        ...
        onDownloadTemplate={() => legalCaseService.downloadDocumentTypeTemplate()}
      />
      ...
      {modal === 'create' && (
        <Modal title="Tambah Jenis Dokumen" onClose={closeModal}>
          <form onSubmit={form.handleSubmit(onCreateSubmit)} className="space-y-4">
            <Field label="Nama" error={form.formState.errors.name?.message}>
              <Input {...form.register('name')} placeholder="Nama (slug)" />
            </Field>
            <Field label="Label" error={form.formState.errors.label?.message}>
              <Input {...form.register('label')} placeholder="Label tampilan" />
            </Field>
